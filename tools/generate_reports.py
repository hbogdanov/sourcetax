...
def generate():
    ...
    qb = exporter.generate_quickbooks_csv(
        out_path="artifacts/exports/quickbooks_import.csv", db_path="data/interim/store.db"
    )
    print("QuickBooks CSV:", qb)
    
    print("Computing Schedule C totals...")
    totals, counts = exporter.compute_schedule_c_totals(db_path="data/interim/store.db")
    sc = exporter.write_schedule_c_csv(
        totals, count_by_category=counts, out_path="artifacts/exports/schedule_c_totals.csv"
    )
    print("Schedule C totals:", sc)
    
    print("Exporting audit pack...")
    audit = exporter.export_audit_pack(db_path="data/interim/store.db")
    print("Audit pack:", audit)
    
    print("\nExport Metrics:")
    metrics = exporter.export_metrics(db_path="data/interim/store.db")
